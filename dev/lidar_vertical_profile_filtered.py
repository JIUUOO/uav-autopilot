...
# LiDAR vertical profile mapper with terrain-sector filtering
...
#
# It saves:
#   1. vertical_profile_filtered_points.png
#   2. vertical_profile_filtered_hit_map.png
...
POINTS_PNG = "dev/maps/vertical_profile_filtered_points.png"
MAP_PNG = "dev/maps/vertical_profile_filtered_hit_map.png"
...
def collect_vertical_points(laser, num_scans):
    ...
    while collected < num_scans and ydlidar.os_isOk():
        ...
        for i in range(n):
            ...
            # Main filter: keep only the terrain-relevant angle sector.
            ...
            # Secondary filter: remove remaining upper-side arc artifacts.
            if z > MAX_UPPER_Z:
        ...
        collected += 1
        print(
            f"[scan {collected}/{num_scans}] "
            f"raw={n}, "
            f"valid={valid_count}, "
            f"range_reject={range_reject_count}, "
            f"sector_reject={sector_reject_count}, "
            f"upper_reject={upper_reject_count}"
        )
...
def save_points_plot(points_xz, out_path):
    ...
    plt.xlabel("scan x (m)")
    plt.ylabel("scan z (m)")
    plt.title("Filtered LiDAR Vertical Scan Profile")
...
def save_hit_grid(grid, out_path):
    ...
    plt.xlabel("scan x (m)")
    plt.ylabel("scan z (m)")
    plt.title(f"Filtered LiDAR Vertical Hit Map ({MAP_RESOLUTION:.2f} m/cell)")
    plt.savefig(out_path, dpi=200, bbox_inches="tight")
    plt.close()
