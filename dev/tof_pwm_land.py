...
# Motor PWM throttle up 후 tof 50cm 이상이 되면 Pixhawk LAND 모드로 자동 착륙
...
# ToF 설정
TOF_THRESHOLD_CM = 50  # 50cm 이상이면 LAND 모드 전환
TOF_TRIGGER_COUNT = 3  # 연속 3번 이상 넘으면 trigger
TOF_TIMEOUT_SEC = 30  # 최대 테스트 시간

# LAND 이후 설정
LAND_DISARM_HEIGHT_CM = 15  # 이 거리 이하로 내려오면 disarm 시도
LAND_TIMEOUT_SEC = 20  # LAND 진입 후 최대 대기 시간
...
def set_flight_mode(conn, mode_name):
    mode_mapping = conn.mode_mapping()

    if mode_name not in mode_mapping:
        raise RuntimeError(
            f"Mode {mode_name} not available. Available: {list(mode_mapping.keys())}"
        )
    ...
    time.sleep(1)
    print(f"Flight mode set to {mode_name}")
...
def land_sequence(conn):
    print("LAND sequence triggered")

    # 1. 더 이상 throttle override를 넣지 않음
    #    LAND 모드가 자체적으로 모터 출력을 제어해야 하므로 override를 해제한다.
    clear_rc_override(conn)
    time.sleep(0.2)

    # 2. Pixhawk LAND 모드 전환
    set_flight_mode(conn, "LAND")

    # 3. LAND 중 ToF 거리 모니터링
    start = time.time()
    last_print = 0
    ...
    while time.time() - start < LAND_TIMEOUT_SEC:
        ...
        if msg is None:
            print("LAND: No DISTANCE_SENSOR message")
            continue
        ...
        if now - last_print >= 0.5:
            last_print = now
            print(f"LAND: dist={dist_cm}cm")

        if dist_cm <= LAND_DISARM_HEIGHT_CM:
            print(
                f"LAND: near ground detected ({dist_cm}cm <= {LAND_DISARM_HEIGHT_CM}cm)"
            )
            time.sleep(2)
            disarm(conn)
            return True

    print("LAND timeout reached. Sending disarm command.")
    disarm(conn)
    return False
